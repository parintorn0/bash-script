install () {
    ...
    echo "Downloading..."
    curl -sL "https://github.com/zen-browser/desktop/releases/latest/download/zen.linux-x86_64.tar.xz" > /tmp/zen.linux-x86_64.tar.xz
    echo "Downloaded"
    echo ""
    echo ""
    echo "Extracting..."
    sudo tar xf /tmp/zen.linux-x86_64.tar.xz -C /opt/
    if [ -e /tmp/zen.linux-x86_64.tar.xz ]; then
    	sudo rm /tmp/zen.linux-x86_64.tar.xz
    fi
    echo "Extracted"
    echo ""
    echo ""
...
uninstall () {
    if [ -d /opt/zen ]; then
        echo "Let uninstaller use root user account"
        sudo rm -R /opt/zen/
        echo "Uninstalled"
        echo ""
        echo ""
        ...
        echo ""
        echo ""
        echo "Uninstallation Conpleted"
    fi
}


echo "Zen Browser Installer"
echo ""
echo ""
read -p "install or remove (i, r): " charInput
case $charInput in
    I | i)
        install
        ;;
