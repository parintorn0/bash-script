...
install () {
    ...
    echo "Downloading..."
    curl -sL $SOURCE_URL > /tmp/source$BUILD_FORMAT
    echo "Downloaded"
    echo "Extracting..."
    sudo tar xf /tmp/source$BUILD_FORMAT -C /opt/
    if [ -e /tmp/source$BUILD_FORMAT ]; then
    	sudo rm /tmp/source$BUILD_FORMAT
    fi
    echo "Extracted"
    echo "$DESKTOP_ENTRY" | sudo tee $DESKTOP_ENTRY_PATH > /dev/null
...
echo "1: Firefox ESR"
echo "2: Zen Browser"
echo "3: Floorp Browser"

read -p "Select Browser to install or uninstall: " BROWSER

zen () {
    DEST_PATH="/opt/zen"
    SOURCE_URL=$(curl -s https://api.github.com/repos/zen-browser/desktop/releases/latest | jq -r '.assets[] | select(.name | test("x86_64\\.tar\\.xz$")) | .browser_download_url')
    BUILD_FORMAT=".tar.xz"
    DESKTOP_ENTRY_PATH="/usr/share/applications/zen.desktop"
    ...
    Exec=/opt/zen/zen-bin --ProfileManager"
}

floorp () {
    DEST_PATH="/opt/floorp"
    SOURCE_URL=$(curl -s https://api.github.com/repos/Floorp-Projects/Floorp/releases/latest | jq -r '.assets[] | select(.name | test("x86_64\\.tar\\.bz2$")) | .browser_download_url')
    BUILD_FORMAT=".tar.bz2"
    DESKTOP_ENTRY_PATH="/usr/share/applications/floorp.desktop"
    DESKTOP_ENTRY="[Desktop Entry]
    Version=1.0
    ...
    Name[zh_CN]=新建窗口
    Name[zh_TW]=開啟新視窗
    Exec=/opt/floorp/floorp-bin -new-window
    ...
    Exec=/opt/floorp/floorp-bin -private-window
    "
}

firefox_esr () {
    DEST_PATH="/opt/firefox"
    SOURCE_URL="https://download.mozilla.org/?product=firefox-esr-latest-ssl&os=linux64&lang=en-US"
    BUILD_FORMAT=".tar.bz2"
    DESKTOP_ENTRY_PATH="/usr/share/applications/firefox-esr.desktop"
    DESKTOP_ENTRY="[Desktop Entry]
    Version=1.0
    Type=Application
    ...
    X-GNOME-FullName[pa_IN]=Firefox ਵੈੱਬ ਬਰਾਊਜ਼ਰ
    ...
    [Desktop Action new-window]
    Exec=/opt/firefox/firefox-bin --new-window %u
    Name=New Window
    Name[ach]=New Window
    Name[af]=New Window
    Name[an]=New Window
    ...
    [Desktop Action new-private-window]
    Exec=/opt/firefox/firefox-bin --private-window %u
    ...
    Exec=/opt/firefox/firefox-bin --ProfileManager
